...
Search.searchUserSessions = (value, page, result) => {
    ...
    if (page != 0)
        offset = page * 10;
    let query = "SELECT *, CASE WHEN enddate >= NOW() THEN 1 ELSE 0 END AS expiration FROM course_session WHERE (session_name LIKE ? OR description like ? OR location like ? OR tags like ?)  AND is_trash != 1 ORDER BY id DESC limit ?,10";
