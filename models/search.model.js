...
Search.All = (value, page,id, result) => {
    ...
    if (page != 0)
        offset = page * 10;
    let query = "SELECT * FROM((SELECT courses.id AS id,courses.name AS title, courses.duration AS duration,courses.price AS price, 'Online' AS location,courses.tags,courses.img AS img, courses.rating AS rating, 'course' AS stype, true AS showRate FROM courses WHERE (name LIKE ? OR description like ? OR code like ? OR tags like ?) AND is_trash != 1 ) UNION ALL (SELECT users.user_id AS id,users.fullname AS title, NULL AS duration, users.hour_price AS price,users.address, users.tags, users.picture AS img, users.rating AS rating, 'user' AS stype, true AS showRate FROM users WHERE (fullname LIKE ? OR tags like ? OR about like ?) AND users.type != 'student' AND users.user_id != ? AND users.tags IS NOT NULL   AND is_trash != 1 AND is_active = 1 ) UNION ALL (SELECT course_session.id AS id, course_session.session_name AS title, course_session.duration AS duration,course_session.price AS price, course_session.location AS location,course_session.tags, course_session.img AS img, null AS rating, 'session' AS stype, false AS showRate FROM course_session WHERE (session_name LIKE ? OR description like ? OR location like ? OR tags like ?) AND user_id != ? AND startdate > NOW()   AND is_trash != 1 ))A LIMIT ?,10";
...
Search.searchSessions = (value, id, page, result) => {
    ...
    if (page != 0)
        offset = page * 10;
    let query = "SELECT course_session.id AS id, course_session.session_name AS title, course_session.duration AS duration,course_session.price AS price, course_session.location AS location,course_session.tags, course_session.img AS img, null AS rating, 'session' AS stype, false AS showRate FROM course_session WHERE (session_name LIKE ? OR description like ? OR location like ? OR tags like ?) AND user_id != ? AND startdate > NOW() AND is_trash != 1 limit ?,10";
