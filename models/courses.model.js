...
Courses.fetchAllNewCourses = (email,result) => {
    let query = "SELECT *  FROM courses LEFT JOIN users ON users.user_id = courses.user_id where courses.is_confirmed = 1 AND users.email != ? AND courses.is_trash != 1 ORDER BY id DESC LIMIT 0,5";
