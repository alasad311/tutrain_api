...
User.getAllOrders = (id, page, result) => {
    ...
    if (page != 0)
        offset = page * 6;
    sql.query("SELECT *, orders.id AS ORDERID, CASE WHEN rating.id IS NULL THEN 0 ELSE 1 END AS is_rated FROM orders LEFT JOIN courses ON courses.id = orders.course_id LEFT JOIN schedule ON schedule.tbooking_id = orders.book_id LEFT JOIN users ON users.user_id = schedule.tutor_id LEFT JOIN course_session ON course_session.id = orders.session_id LEFT JOIN rating ON rating.order_id = orders.order_id WHERE orders.user_id = ? ORDER BY orders.date DESC LIMIT ?,6", [id, offset], (err, res) => {
        if (err) {
            console.log("error: ", err);
            result(null, err);
            return;
