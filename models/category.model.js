...
Category.getAllCourses = (userID,id,page,result) => {
    ...
    if (page != 0)
        offset = page * 10;
    let query = "SELECT * FROM courses LEFT JOIN users ON users.user_id = courses.user_id where cat_id = ? AND courses.user_id != ?  AND courses.user_id IS NOT NULLLIMIT ?,10 ";
