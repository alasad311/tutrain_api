...
exports.searchCourses = (req, res) => {
  ...
  Search.Courses(req.params.value,req.params.page,userID,(err, data) => {
    if (err)
      res.status(200).send({
        code:err.code,
      });
...
exports.searchUserSessions = (req, res) => {
  const authHeader = req.headers['authorization']
  const token = authHeader && authHeader.split(' ')[1]  
  const userID = req.headers['userid']
  ...
  Search.searchUserSessions(req.params.value,req.params.page,userID,(err, data) => {
    if (err)
      res.status(200).send({
        code:err.code,
      });
...
exports.searchUserCourses = (req, res) => {
  const authHeader = req.headers['authorization']
  const token = authHeader && authHeader.split(' ')[1]  
  const userID = req.headers['userid']
  ...
  Search.searchUserCourses(req.params.value,req.params.page,userID,(err, data) => {
    if (err)
      res.status(200).send({
        code:err.code,
      });
